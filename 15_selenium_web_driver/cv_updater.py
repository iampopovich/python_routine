...
def send_reply(browser, config):
    try:
        ...
        browser.get(
            'https://spb.hh.ru/search/vacancy?order_by=publication_time&clusters=true&resume={}&enable_snippets=true'.format(resume_id))
        time.sleep(10)
...
def main():
    while True:
        try:
            ...
            if config['reply'] != 0:
                # send_reply(browser, config) #unstable fusnctionality will be tested and deployed in next version
                pass
            browser.quit()
            # ласно правилам ХХ можно апдейтить CV  раз в 4 часа
            time.sleep(14450)
        except Exception as ex:
            browser.quit()
            raise ex
