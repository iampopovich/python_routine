...
class ColoredBlock:
	...
	def calculateScalePoint(self, parent, view,text,scale = 1.000):
		...
		#vectors = [x_max - x_min, y_max - y_min,z_max - z_min]
		#startPosition##########
	...
	def createTechRequirements(self, parent):
		...
		if self.isDrafting:
			...
			if self.checkView(self) is None:
				...
				layout.ReplaceView(self.workPart.ModelingViews.WorkView, modelingView, True)
			else: modelingView = self.checkView(self)
			text = text.split('\r\n')
	...
	def editTechRequirements(self, parent):
		try:
			modelingView = self.checkView(self)
			noteTag = self.checkNote(self,modelingView)
			if modelingView is None:
				self.theUI.NXMessageBox.Show('__alarm__template__', NXOpen.NXMessageBox.DialogType.Warning, 'Вид отсутствует')
				return None
			else: pass
			if noteTag is None:
				self.theUI.NXMessageBox.Show('__alarm__template__', NXOpen.NXMessageBox.DialogType.Warning, 'ТТ отсутствуют')
				return None
			else: pass
			...
			#etr_subprocess = subprocess.Popen([self.theExecutablePath],input=text, stdout = subprocess.PIPE, stdin = subprocess.PIPE)
			...
			self.theUI.NXMessageBox.Show('__alarm__template__', NXOpen.NXMessageBox.DialogType.Warning, 'editTechReqs failed with %s' %ex)
	...
	def checkNote(self,parent,view = None, noteTag = None):
		pmiNotes = [item for item in view.AskVisibleObjects() if isinstance(item,NXOpen.Annotations.PmiNote)]
		noteTag = [item.Tag for item in pmiNotes if item.Name in ['ТЕХНИЧЕСКИЕ_ТРЕБОВАНИЯ']]
		return noteTag[0]

	def removeTechRequirements(self, parent):
		noteTag = self.checkNote(self)
		if noteTag is None:
			self.theUI.NXMessageBox.Show('__alarm__template__', NXOpen.NXMessageBox.DialogType.Warning, 'ТТ отсутствуют.')
			return None
		else:
			...
			self.theSession.UpdateManager.AddToDeleteList([taggedNote])
			self.theUI.NXMessageBox.Show('__alarm__template__', NXOpen.NXMessageBox.DialogType.Warning, 'ТТ удалены.')	
